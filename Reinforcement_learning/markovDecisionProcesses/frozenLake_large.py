...
print(np.reshape(random_map, (16, 1)))
...
# PI
pi = mdp.PolicyIteration(T, R, gamma=0.99, max_iter=100)
pi.run()
# Plot convergence
fig = plt.figure()
ax1 = fig.add_subplot(111)
...
# for i in range(len(epsilon)):
#     np.random.seed(SEED)
#     ql = mdp.QLearning(T, R, gamma=0.99, alpha=0.9, epsilon=epsilon[i], n_iter=400000)
#     ql.run()
...
#     np.random.seed(SEED)
#     ql = mdp.QLearning(T, R, gamma=0.99, alpha=alpha[i], epsilon=1, n_iter=400000)
#     ql.run()
...
ql = mdp.QLearning(T, R, gamma=0.99, alpha=0.9, epsilon=1, n_iter=400000)
ql.run()
